...
class GR_MAPPOLPolicy:
    ...
    def __init__(
        ...
        self.cost_critic = Aps_GR_Critic(
            args,
            self.share_obs_space,
            self.device,
            self.split_batch,
    ...
    def get_actions(
        ...
        cost_preds, rnn_states_cost = self.cost_critic.forward(
            batch, rnn_states_cost, masks
        )

        return (values, cost_preds, actions, action_log_probs, rnn_states_actor, rnn_states_critic, rnn_states_cost)
    ...
    def get_cost_values(self, graph_batch, rnn_states_cost, masks):
        """
        ...
        :return values: (torch.Tensor) value function predictions.
        """
        cost_preds, _ = self.cost_critic(graph_batch, rnn_states_cost, masks)
        return cost_preds
    ...
    def evaluate_actions(
        ...
        cost_values, _ = self.cost_critic.forward(
            graph_batch, rnn_states_cost, masks
        )
