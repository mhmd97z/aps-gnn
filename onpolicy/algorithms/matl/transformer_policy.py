...
class TransformerPolicy:
    ...
    def __init__(self, args, obs_space, cent_obs_space, act_space, num_agents, device=torch.device("cpu")):
        ...
        if self.algorithm_name in ["matl"]:
            from onpolicy.algorithms.matl.ma_transformer import MultiAgentTransformer as MAT
    ...
    def get_actions(self, cent_obs, obs, rnn_states_actor, rnn_states_critic, rnn_states_critic_cost, masks, available_actions=None,
                    deterministic=False):
        """
        ...
        actions, action_log_probs, values, cost_values = self.transformer.get_actions(cent_obs,
                                                                         obs,
                                                                         available_actions,
                                                                         deterministic)
        ...
        rnn_states_critic_cost = check(rnn_states_critic_cost).to(**self.tpdv)
        return values, cost_values, actions, action_log_probs, rnn_states_actor, rnn_states_critic, rnn_states_critic_cost
    ...
    def get_cost_values(self, cent_obs, obs, rnn_states_critic_cost, masks, available_actions=None):
        """
        Get cost value function predictions.
        :param cent_obs (np.ndarray): centralized input to the critic.
        :param rnn_states_critic_cost: (np.ndarray) if critic is RNN, RNN states for critic.
        :param masks: (np.ndarray) denotes points at which RNN states should be reset.

        :return cost_values: (torch.Tensor) cost value function predictions.
        """
        ...
        cost_values = self.transformer.get_values(cent_obs, obs, available_actions)

        cost_values = cost_values.view(-1, 1)

        return cost_values

    def evaluate_actions(self, cent_obs, obs, rnn_states_actor, rnn_states_critic, rnn_states_critic_cost, actions, masks,
        ...
        action_log_probs, values, cost_values, entropy = self.transformer(cent_obs, obs, actions, available_actions)
        ...
        return values, cost_values, action_log_probs, entropy
