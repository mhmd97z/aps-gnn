...
class MultiAgentTransformer(nn.Module):
    ...
    def __init__(self, state_dim, obs_dim, action_dim, n_agent,
        ...
        self.cost_encoder = Encoder(state_dim, obs_dim, n_block, n_embd, n_head, n_agent, encode_state)
        ...
        self.fuse_rep = nn.Linear(2 * n_embd, n_embd)
    ...
    def forward(self, state, obs, action, available_actions=None):
        ...
        c_loc, obs_rep_c = self.cost_encoder(state, obs)
        
        obs_rep_combined = torch.cat([obs_rep, obs_rep_c], dim=-1)
        obs_rep_fused = self.fuse_rep(obs_rep_combined)


        if self.action_type == 'Discrete':
            action = action.long()
            action_log, entropy = discrete_parallel_act(self.decoder, obs_rep_fused, obs, action, batch_size,
                                                        self.n_agent, self.action_dim, self.tpdv, available_actions)
        else:
            action_log, entropy = continuous_parallel_act(self.decoder, obs_rep_fused, obs, action, batch_size,
                                                          self.n_agent, self.action_dim, self.tpdv)

        return action_log, v_loc, c_loc, entropy
    ...
    def get_actions(self, state, obs, available_actions=None, deterministic=False):
        ...
        if self.action_type == "Discrete":
            output_action, output_action_log = discrete_autoregreesive_act(self.decoder, obs_rep_fused, obs, batch_size,
                                                                           self.n_agent, self.action_dim, self.tpdv,
                                                                           available_actions, deterministic)
        else:
            output_action, output_action_log = continuous_autoregreesive_act(self.decoder, obs_rep_fused, obs, batch_size,
                                                                             self.n_agent, self.action_dim, self.tpdv,
                                                                             deterministic)

        return output_action, output_action_log, v_loc, c_loc
    ...
    def get_values(self, state, obs, available_actions=None):
        ...
        state = check(state).to(**self.tpdv)
        obs = check(obs).to(**self.tpdv)
        v_tot, _ = self.encoder(state, obs)

        return v_tot

    def get_cost_values(self, state, obs, available_actions=None):
        # state unused
        ori_shape = np.shape(state)
        state = np.zeros((*ori_shape[:-1], 37), dtype=np.float32)
        ...
        state = check(state).to(**self.tpdv)
        obs = check(obs).to(**self.tpdv)
        c_tot, _ = self.cost_encoder(state, obs)
