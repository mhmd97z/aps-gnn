...
from onpolicy.envs.aps.aps import Aps_c
from onpolicy.envs.env_wrappers import ApsCSubprocVecEnv
from onpolicy.runner.shared.apsc_runner import ApsCRunner as Runner

def make_train_env(all_args, if_graph=True):
    def get_env_fn(rank):
        def init_env():
            if all_args.env_name == "aps":
                env = Aps_c(all_args.env_args, if_graph=if_graph)
            else:
    ...
    return ApsCSubprocVecEnv([get_env_fn(i) for i in range(all_args.n_rollout_threads)])
