...
class MatlReplayBuffer(object):
    ...
    def __init__(self, args, num_agents, obs_space, cent_obs_space, act_space, env_name):
        ...
        self.cost_preds = np.zeros(
            (self.episode_length + 1, self.n_rollout_threads, num_agents, 1),
            dtype=np.float32,
        )
        self.returns = np.zeros_like(self.value_preds)
        self.cost_returns = np.zeros_like(self.cost_preds)
    ...
    def insert(self, share_obs, obs, rnn_states_actor, rnn_states_critic, rnn_states_cost, actions, action_log_probs,
               value_preds, cost_preds, rewards, costs, masks, bad_masks=None, active_masks=None, available_actions=None):
        """
    ...
    def compute_cost_returns(self, next_cost, cost_normalizer=None):
        """
        Compute returns either as discounted sum of costs, or using GAE.
        :param next_cost: (np.ndarray) cost predictions for the step after the last episode step.
        :param cost_normalizer: (PopArt) If not None, PopArt cost normalizer instance.
        """
        self.cost_preds[-1] = next_cost
        gae = 0
        for step in reversed(range(self.costs.shape[0])):
            if self._use_popart or self._use_valuenorm:
                delta = self.costs[step] + self.gamma * cost_normalizer.denormalize(
                    self.cost_preds[step + 1]) * self.masks[step + 1] \
                        - cost_normalizer.denormalize(self.cost_preds[step])
                gae = delta + self.gamma * self.gae_lambda * self.masks[step + 1] * gae
                ...
                self.advantages[step] = gae
                self.returns[step] = gae + cost_normalizer.denormalize(self.cost_preds[step])
            else:
                delta = self.costs[step] + self.gamma * self.cost_preds[step + 1] * \
                        self.masks[step + 1] - self.cost_preds[step]
                gae = delta + self.gamma * self.gae_lambda * self.masks[step + 1] * gae

                self.cost_advantages[step] = gae
                self.cost_returns[step] = gae + self.cost_preds[step]

    def feed_forward_generator_transformer(self, advantages, cost_advantages, num_mini_batch=None, mini_batch_size=None):
        """
        ...
        rnn_states_cost = rnn_states_cost[rows, cols]
        ...
        cost_preds = self.cost_preds[:-1].reshape(-1, *self.cost_preds.shape[2:])
        cost_preds = cost_preds[rows, cols]
        returns = self.returns[:-1].reshape(-1, *self.returns.shape[2:])
        returns = returns[rows, cols]
        ue_idx = self.ue_idx[:-1].reshape(-1, 1)
        # ue_idx = ue_idx[rows, cols]
        cost_returns = self.cost_returns[:-1].reshape(-1, *self.cost_returns.shape[2:])
        cost_returns = cost_returns[rows, cols]
        ...
        cost_advantages = cost_advantages.reshape(-1, *cost_advantages.shape[2:])
        cost_advantages = cost_advantages[rows, cols]
        ...
        for indices in sampler:
            ...
            rnn_states_cost_batch = rnn_states_cost[indices].reshape(-1, *rnn_states_cost.shape[2:])
            ...
            cost_preds_batch = cost_preds[indices].reshape(-1, *cost_preds.shape[2:])
            
            return_batch = returns[indices].reshape(-1, *returns.shape[2:])
            cost_return_batch = cost_returns[indices].reshape(-1, *cost_returns.shape[2:])
            ...
            if cost_advantages is None:
                ...
                cost_adv_targ = cost_advantages[indices].reshape(-1, *cost_advantages.shape[2:])
            ue_idx_batch = ue_idx[indices]

            yield share_obs_batch, obs_batch, rnn_states_batch, rnn_states_critic_batch, rnn_states_cost_batch, actions_batch, \
                  ...
                  adv_targ, cost_adv_targ, available_actions_batch, ue_idx_batch
